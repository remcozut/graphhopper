...
public class FinishInstruction extends Instruction {
    public FinishInstruction(String name, final double lat, final double lon, final double ele) {
        super(FINISH, TurnType.ARRIVE, name, InstructionAnnotation.EMPTY, new PointList(2, !Double.isNaN(ele)) {
            {
                add(lat, lon, ele);
            }
        });
    ...
    public FinishInstruction(final double lat, final double lon, final double ele) {
        super(FINISH, TurnType.ARRIVE, "", InstructionAnnotation.EMPTY, new PointList(2, !Double.isNaN(ele)) {
            {
                add(lat, lon, ele);
            }
        });
