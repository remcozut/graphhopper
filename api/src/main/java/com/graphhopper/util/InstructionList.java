...
public class InstructionList extends AbstractList<Instruction> {
    ...
    public Instruction merge(int index) {
        if (index == 0) {
            return instructions.get(0);
        }

        Instruction instruction = instructions.get(index - 1);
        Instruction instr = instructions.get(index);

        if (!instruction.isForceKeep() && instruction.mergedStraight <= 3 && instruction.getDistance() <= Helper.MAX_INSTRUCTION_DISTANCE_THRESHOLD) {
            if (! instr.name.isEmpty()) {
                instruction.setName(instr.name);
            }
            if (! instr.annotation.isEmpty()) {
                instruction.setAnnotation(instr.annotation);
            }
            for (String s : instr.getExtraInfoJSON().keySet()) {
                instruction.setExtraInfo(s, instr.getExtraInfoJSON().get(s));
            }
...
//                instruction.setAnnotation(instr.annotation);
        ...
        instruction.nodes.putAll(instr.nodes);

        if (instr.getSign() == 0)  {
            instruction.mergedStraight++;
        }
        instruction.mergedStraight += instr.mergedStraight;
    ...
    public Instruction mergeLast(Instruction instr) {
