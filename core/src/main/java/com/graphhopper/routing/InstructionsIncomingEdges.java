 ...
 * - the allowedIncomingEdges are B->C => return value of {@link #nrOfAllowedIncomingEdges()} is 1
 * - the allIncomingEdges are B->X and B->C => return values of {@link #nrOfAllIncomingEdges()} is 2
 *
 * @author Robin Boldt
 */
class InstructionsIncomingEdges {

    final EdgeIteratorState prevEdge;
    final EdgeIteratorState currentEdge;

    // Outgoing edges that we would be allowed to turn on
    final List<EdgeIteratorState> allowedIncomingEdges;

    // All outgoing edges, including oneways in the wrong direction
    final List<EdgeIteratorState> allIncomingEdges;
    ...
    public InstructionsIncomingEdges(EdgeIteratorState prevEdge,
        ...
        allIncomingEdges = new ArrayList<>();
        allowedIncomingEdges = new ArrayList<>();
        ...
        while (edgeIter.next()) {
            if (edgeIter.getAdjNode() == prevNode && edgeIter.getAdjNode() != adjNode) {
                ...
                allIncomingEdges.add(tmpEdge);
                if (tmpEdge.get(accessEnc)) {
                    allowedIncomingEdges.add(tmpEdge);
                }
            }
        }
    }
     ...
     */
    public int nrOfAllowedIncomingEdges() {
        return allowedIncomingEdges.size();
    }
     ...
     */
    public int nrOfAllIncomingEdges() {
        return allIncomingEdges.size();
    }
     ...
     */
    public boolean incomingEdgesAreSlowerByFactor(double factor) {
        ...
        for (EdgeIteratorState edge : allIncomingEdges) {
    ...
    public EdgeIteratorState getOtherContinue(double prevLat, double prevLon, double prevOrientation) {
        int tmpSign;
        for (EdgeIteratorState edge : allowedIncomingEdges) {
    ...
    public List<EdgeIteratorState> getAllowedIncomingEdges() {
        return allowedIncomingEdges;
    }
